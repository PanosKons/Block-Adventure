...
void Block::OnBreak(BLOCK_ID id)
{
	...
	auto b = &(*a)[Position.x % ChunkSize][Position.y][Position.z % ChunkSize];
...
void Block::Update()
{
	...
	if (blocks[0] != nullptr)
		if (blocks[0]->GetBlockId() == BLOCK_ID::Air)
		{
	...
	if (blocks[1] != nullptr)
		if (blocks[1]->GetBlockId() == BLOCK_ID::Air)
		{
	...
	if (blocks[2] != nullptr)
		if (blocks[2]->GetBlockId() == BLOCK_ID::Air)
		{
	...
	if (blocks[3] != nullptr)
		if (blocks[3]->GetBlockId() == BLOCK_ID::Air)
		{
	...
	if (blocks[4] != nullptr)
		if (blocks[4]->GetBlockId() == BLOCK_ID::Air)
		{
	...
	if (blocks[5] != nullptr)
		if (blocks[5]->GetBlockId() == BLOCK_ID::Air)
		{
